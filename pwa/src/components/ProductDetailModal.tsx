...
export const ProductDetailModal = ({
  ...
  const price = formatPrice(parseFloat(product.price), settings.default_currency || "TRY", currentLanguage)

  const metaItems = [
    product.weight
      ? { key: "weight", label: `${product.weight}${t("menu.weight")}` }
      : null,
    product.portion_size
      ? { key: "portion", label: `${t("menu.portion")}${product.portion_size}`, icon: Users }
      : null,
    ...
    product.spice_level && parseInt(product.spice_level) > 0
      ? {
          key: "spice",
          label: `${t("menu.spiceLevel")} ${product.spice_level}/3`,
          icon: Flame,
          accent: true,
        }
      : null,
  ...
  const hasOrigin = Boolean(product.origin_country && product.origin_country.trim() !== "")
  const hasAdditives = Boolean(product.additives && product.additives.length > 0)

  return (
    <MenuBottomSheet open={isOpen} onOpenChange={onOpenChange} title={title} tall>
      <div className="space-y-0 pb-4">
        <div className="-mx-5 relative overflow-hidden bg-muted">
          {outOfStock ? (
          ...
          <img
            ...
            className={cn("aspect-[4/3] w-full object-cover", outOfStock && "opacity-70 grayscale")}
          />
        </div>
        ...
        {product.tags && product.tags.length > 0 ? (
          <div className="flex flex-wrap gap-2 border-b border-border/60 py-4">
            {product.tags.map((tag) => (
              <span
                key={tag}
                className="rounded-full border border-border/60 bg-card px-3 py-1 text-xs font-medium text-foreground"
              >
                {t(`tags.${tag}`)}
              </span>
            ))}
        ...
        <Accordion type="multiple" className="w-full">
          ...
          {hasOrigin ? (
            <AccordionItem value="origin" className="border-b border-border/60">
              <AccordionTrigger className="py-4 text-sm font-semibold hover:no-underline">
                {t("menu.origin")}
              </AccordionTrigger>
              <AccordionContent className="pb-4">
