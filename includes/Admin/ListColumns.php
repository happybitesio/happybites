...
final class ListColumns {
    ...
    public function register() {
        ...
        $this->loader->add_action('admin_footer', $this, 'make_taxonomy_single_select');
    }
        ...
        switch ($column) {
            case 'thumbnail':
                if (has_post_thumbnail($post_id)) {
                    ...
                    echo '<span style="color: #999; font-style: italic;">' . __('No image', 'happybites') . '</span>';
                }
                break;
            ...
            case 'menu_category_selection':
                ...
                if (!empty($selected_category)) {
                    ...
                    if ($category && !is_wp_error($category)) {
                        ...
                        echo '<em>' . __('Category not found', 'happybites') . '</em>';
                    }
                } else {
                    echo '<em>' . __('No category selected', 'happybites') . '</em>';
                }
                break;
            ...
            case 'menu_spice_level':
                ...
                if ($spice_level !== '') {
                    $spice_levels = array(
                        ...
                        '4' => __('4 - Very Spicy', 'happybites'),
                        '5' => __('5 - Extremely Spicy', 'happybites')
                    );
                    echo isset($spice_levels[$spice_level]) ? $spice_levels[$spice_level] : $spice_level;
                }
                break;
    ...
    /**
     * Taxonomy tek seçim yapma
     */
    public function make_taxonomy_single_select() {
        global $post_type;
        if ($post_type !== 'happybites_menu_item') return;
        ?>
        <script>
            jQuery(document).ready(function($) {
                // Tek kategori seçimine izin ver (her yerde)
                function enforceSingleCategorySelection() {
                    const selector = 'input[name="tax_input[happybites_menu_category][]"]';
                ...
                enforceSingleCategorySelection(); // İlk yükleme

                // "En Çok Kullanılanlar" sekmesi ilk defa tıklanınca yüklendiği için, o tıklamayı yakalayıp tekrar kontrol bağlamalıyız
                if (document.getElementById('category-tabs')) {
                    $('#category-tabs a').on('click', function () {
                        setTimeout(() => {
                            enforceSingleCategorySelection();
                        }, 200); // biraz gecikmeyle bağla (Ajax tamamlanana kadar bekle)
                    });
                }

                // Ekstra sağlamlık için MutationObserver da tutalım (son çare)
                const observer = new MutationObserver(() => enforceSingleCategorySelection());
