...
final class MenuManagementService
{
    ...
    public function get_tree(): array
    {
        ...
        $all_products = get_posts([
            ...
            // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query -- order products by custom menu order.
            'meta_query' => [
                'relation' => 'OR',
                'menu_order_clause' => [
                    'key' => '_menu_order',
        ...
        $build_subcategories = function (int $parent_id) use (&$terms_by_parent, &$products_by_cat): array {
            ...
            foreach ($subs as $sub) {
                ...
                $sub_data = [
                    ...
                    'image_id' => $this->category_image_id((int) $sub->term_id),
                    'image' => $this->category_image_url((int) $sub->term_id),
                    'products' => [],
                ];
        ...
        foreach ($top_categories as $cat) {
            ...
            $category_data = [
                ...
                'image_id' => $this->category_image_id((int) $cat->term_id),
                'image' => $this->category_image_url((int) $cat->term_id),
                'products' => [],
            ...
            if (!empty($products_by_cat[$cat->term_id])) {
                ...
                usort($category_data['products'], static fn ($a, $b) => ($a['order'] ?? 0) <=> ($b['order'] ?? 0));
            }
        ...
        usort($uncategorized_data, static fn ($a, $b) => ($a['order'] ?? 0) <=> ($b['order'] ?? 0));
    ...
    private function category_image_id(int $term_id): ?int
    {
        $image_id = (int) get_term_meta($term_id, CategoryImage::META_KEY, true);

        return $image_id > 0 ? $image_id : null;
    }

    private function category_image_url(int $term_id): string
    {
        $image = CategoryImage::for_api($term_id);
