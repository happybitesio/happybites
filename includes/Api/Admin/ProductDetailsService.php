...
final class ProductDetailsService
{
    ...
    public function map(WP_Post $post): array
    {
        ...
        return [
            ...
            'spice_level' => $this->normalize_spice_level((string) get_post_meta($post->ID, '_menu_spice_level', true)),
    ...
    public function set_image(int $product_id, array $params)
    {
        ...
        if ($content_base64 !== '') {
            $filename = sanitize_file_name((string) ($params['filename'] ?? 'product-image.jpg'));
        ...
        } elseif (array_key_exists('image_id', $params) || array_key_exists('image_url', $params) || array_key_exists('content_base64', $params)) {
            delete_post_thumbnail($product_id);
        } else {
            return new \WP_Error('invalid_image', __('Provide content_base64, image_url, or image_id.', 'happybites'));
        }
    ...
    public function save(int $product_id, array $params)
    {
        ...
        update_post_meta($result_id, '_menu_spice_level', $this->normalize_spice_level((string) ($params['spice_level'] ?? '')));
        ...
        if (!empty($params['image_touched']) || !empty($params['image_url']) || !empty($params['content_base64'])) {
    ...
    private function normalize_spice_level(string $level): string
    {
    ...
    private function next_product_order(int $category_id): int
    {
        ...
        $products = get_posts([
            'post_type' => PostTypes::POST_TYPE,
            'posts_per_page' => 1,
            // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query -- scoped lookup of the last ordered product.
