...
final class RecaptchaVerifier
{
    ...
    public static function verify(string $token, string $action = 'review_submit')
    {
        ...
        if ($token === '') {
            return new \WP_Error('recaptcha_missing', __('reCAPTCHA doğrulaması gerekli.', 'happybites'));
        }
        ...
        if (is_wp_error($response)) {
            return new \WP_Error('recaptcha_request_failed', __('reCAPTCHA doğrulaması başarısız.', 'happybites'));
        }

        $payload = json_decode((string) wp_remote_retrieve_body($response), true);

        if (!is_array($payload) || empty($payload['success'])) {
            return new \WP_Error('recaptcha_invalid', __('reCAPTCHA doğrulaması geçersiz.', 'happybites'));
        }
        ...
        if ($payload_action !== '' && $payload_action !== $action) {
            return new \WP_Error('recaptcha_action_mismatch', __('reCAPTCHA doğrulaması geçersiz.', 'happybites'));
        }

        if ($score < self::MIN_SCORE) {
            return new \WP_Error('recaptcha_low_score', __('Güvenlik doğrulaması başarısız. Lütfen tekrar deneyin.', 'happybites'));
        }
