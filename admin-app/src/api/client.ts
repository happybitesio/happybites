...
export const api = {
  ...
  getMcpSettings: () => request<{ success: true; data: McpSettingsData }>('/admin/mcp'),
  updateMcpSettings: (body: { enabled?: boolean }) =>
    request<{ success: true; data: McpSettingsData }>('/admin/mcp', {
      method: 'PUT',
      body: JSON.stringify(body),
    }),
  rotateMcpToken: () =>
    request<{ success: true; data: McpSettingsData & { token?: string }; message?: string }>(
      '/admin/mcp/rotate',
      { method: 'POST' },
    ),
};
